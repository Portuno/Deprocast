              ...
              <div>
                ...
                <input
                  ...
                  type="text"
										...
										className="w-full px-3 md:px-4 py-3 bg-gray-800/50 border border-gray-600/30 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-transparent text-sm md:text-base"
									/>
								</div>
								<div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
									<div>
										...
										<select
											...
											className="w-full px-3 md:px-4 py-3 bg-gray-800/50 border border-gray-600/30 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-transparent text-sm md:text-base"
										>
              ...
              <div>
									...
									<div className="flex gap-2">
										<input
											...
											className="flex-1 px-3 md:px-4 py-3 bg-gray-800/50 border border-gray-600/30 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-transparent text-sm md:text-base"
										/>
										<button
											onClick={handleAddSkillsFromInput}
											className="px-3 md:px-4 py-3 bg-gray-700/60 hover:bg-gray-600/60 text-gray-200 rounded-lg transition-colors text-sm md:text-base"
                    ...
                    <button
													...
													className="px-2 md:px-3 py-1 rounded-full text-xs bg-blue-500/20 text-blue-200 border border-blue-500/30 hover:bg-blue-500/30"
...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, isOpen, onOpen, onClose, onUpdated, onDeleted }) => {
	...
	return (
		<div className="bg-gray-900/30 backdrop-blur-xl border rounded-xl p-4 md:p-6 border-gray-700/30 hover:bg-gray-800/30 transition-colors cursor-pointer" onClick={onOpen} role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onOpen()} aria-label={`Open ${project.title}`}>
			<div className="flex items-center justify-between mb-3 text-gray-300">
				<div className="flex items-center gap-2">
					<FolderOpen className="w-4 h-4 md:w-5 md:h-5" />
					<div className="font-semibold text-left text-sm md:text-base">{project.title}</div>
				</div>
				<div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
					<button onClick={() => setIsEditing(true)} className="p-1.5 md:p-2 rounded bg-gray-800/50 border border-gray-700/40 hover:bg-gray-700/50" aria-label="Edit">
						<Pencil className="w-3 h-3 md:w-4 md:h-4" />
					</button>
					<button onClick={handleDelete} className="p-1.5 md:p-2 rounded bg-gray-800/50 border border-gray-700/40 hover:bg-gray-700/50" aria-label="Delete">
						<Trash2 className="w-3 h-3 md:w-4 md:h-4 text-red-400" />
					</button>
				</div>
			</div>
			<p className="text-xs md:text-sm text-gray-400 line-clamp-3 mb-3 md:mb-4">{project.description}</p>
			...
			{isOpen && (
				<ModalPortal>
					<div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
						...
						<div className="relative w-full max-w-[1400px] max-h-[90vh] overflow-y-auto overflow-x-hidden bg-gray-900 border border-gray-700 rounded-2xl p-4 md:p-8 mx-auto" onClick={(e) => e.stopPropagation()}>
						<div className="flex items-start justify-between mb-4 md:mb-6">
							<div className="flex items-center gap-2 md:gap-3">
								<FolderOpen className="w-6 h-6 md:w-7 md:h-7 text-gray-300" />
								<h3 className="text-xl md:text-3xl font-semibold text-white leading-tight">{project.title}</h3>
							</div>
						...
						{/* Meta badges */}
						<div className="flex flex-wrap items-center gap-2 mb-4 md:mb-6 text-xs">
						...
						<div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 text-gray-300">
							{/* Left: Description */}
							<div>
								<p className="text-sm text-gray-400 mb-2">Description</p>
								<p className="text-sm md:text-[15px] leading-6 md:leading-7 whitespace-pre-wrap break-words">{project.description || '—'}</p>
							</div>
							{/* Right: Motivation / Obstacles */}
							<div className="space-y-4 md:space-y-6">
								<div>
									<p className="text-sm text-gray-400 mb-2">Motivation</p>
									<p className="text-sm md:text-[15px] leading-6 md:leading-7 whitespace-pre-wrap break-words">{project.motivation || '—'}</p>
								</div>
								<div>
									<p className="text-sm text-gray-400 mb-2">Known Obstacles</p>
									<p className="text-sm md:text-[15px] leading-6 md:leading-7 whitespace-pre-wrap break-words">{project.known_obstacles || '—'}</p>
								</div>
							</div>
						</div>
			...
			{isEditing && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
					<div className="w-full max-w-lg bg-gray-900 border border-gray-700 rounded-xl p-4 md:p-6">
						...
						<div className="space-y-3">
							<input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full px-3 py-2 bg-gray-800/60 border border-gray-700/50 rounded text-white text-sm md:text-base" placeholder="Title" />
							<textarea value={description} onChange={(e) => setDescription(e.target.value)} className="w-full px-3 py-2 bg-gray-800/60 border border-gray-700/50 rounded text-white text-sm md:text-base" rows={4} placeholder="Description" />
							<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
								<input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="px-3 py-2 bg-gray-800/60 border border-gray-700/50 rounded text-white text-sm md:text-base" />
								<select value={category} onChange={(e) => setCategory(e.target.value)} className="px-3 py-2 bg-gray-800/60 border border-gray-700/50 rounded text-white text-sm md:text-base">
							...
							<div className="flex items-center justify-end gap-2 pt-2">
								<button onClick={() => setIsEditing(false)} className="px-3 md:px-4 py-2 bg-gray-800/70 rounded text-gray-300 text-sm md:text-base">Cancel</button>
								<button onClick={handleSave} disabled={saving} className="px-3 md:px-4 py-2 rounded bg-gradient-to-r from-green-500 to-teal-600 text-white text-sm md:text-base">{saving ? 'Saving...' : 'Save'}</button>
							</div>
						</div>
					</div>
				</div>
